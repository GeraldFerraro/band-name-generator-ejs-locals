...
app.post("/submit", (req, res) => {
  ...
  const randomAdjChoice = Math.floor(Math.random() * adj.length + 1);
  const bandAdj = adj[randomAdjChoice];
  const randomNounChoice = Math.floor(Math.random() * noun.length + 1);
  const bandNoun = noun[randomNounChoice];
  res.render("index.ejs", { adjective: bandAdj, noun: bandNoun });
});
